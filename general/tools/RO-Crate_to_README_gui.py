...
"""
File: RO-Crate_to_README_gui.py
...
class ROCrateReadmeBuilder(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RO-Crate README Generator")
    ...
    def _build_ui(self):
        ...
        self.load_button = QPushButton("Load RO-Crate JSON")
        ...
        left_layout.addWidget(QLabel("Loaded RO-Crate JSON"))
        ...
        self.markdown_editor.setPlaceholderText("Load an RO-Crate JSON file to automatically generate a README.")
    ...
    def load_json(self):
        ...
        self.json_view.setPlainText(json.dumps(self.crate, indent=2, ensure_ascii=False))

        try:
            readme = build_readme_from_rocrate(self.crate)
        except Exception as exc:
    ...
    def save_markdown(self):
        text = self.markdown_editor.toPlainText().strip()
        if not text:
    ...
    def save_markdown_as(self):
        ...
        if self.current_markdown_path:
            default_name = self.current_markdown_path
...
def build_readme_from_rocrate(crate_json):
    ...
    entities = {entity.get("@id"): entity for entity in graph if isinstance(entity, dict) and "@id" in entity}
    root = entities.get("./")
    if root is None:
    ...
    sections.append(render_people_section(root, entities))
    sections.append(render_main_entity(root, entities))
    sections.append(render_file_structure(root, entities))
    sections.append(render_metadata_notes(crate_json, root, entities))
...
def render_people_section(root, entities):
    ...
    for role in role_candidates:
        ...
        if formatted_people:
            people_blocks.append(f"### {prettify_term(role)}\n" + "\n".join(formatted_people))
...
def render_metadata_notes(crate_json, root, entities):
    ...
    lines.append("- **Generated by:** This README was generated automatically from the RO-Crate JSON metadata loaded in the application.")
